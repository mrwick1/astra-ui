...
Astra UI is built as a [monorepo using Turborepo](https://turbo.build/repo/docs/handbook/what-is-a-monorepo) on top of the following amazing projects:
...
Open the `astra-ui/packages/core` directory and run `pnpm dev` to start the development server on the core package.

### Storybook

Open the `astra-ui/apps/storybook` directory and run `pnpm dev` to start the Storybook server.
...
Open the `astra-ui/apps/docs` directory and run `pnpm dev` to start the documentation server.
...
Each component should be in its own directory inside `astra-ui/packages/core/src/components`. The [Button](https://github.com/mrwick1/astra-ui/tree/master/packages/core/src/components/Button) component can be used as an example.
...
The CVA file of each component should be placed in `astra-ui/packages/core/src/theme/styles/Component.styles.ts` and should also be exported from `astra-ui/packages/core/src/theme/styles/index.ts`. The `theme.context.tsx` file will need to be adjusted to include the new component.

Each component should be exported from `astra-ui/packages/core/src/index.ts` as well.

### Storybook

To add a new component to Storybook, add a new story file in `astra-ui/apps/storybook/stories` and import the component from `@astra-ui/core`.
